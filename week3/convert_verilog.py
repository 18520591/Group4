...
f = open("/home/thevu/Desktop/CE434/week2/LAB1/hsv.txt","r")
for i in range(0, height):
    for j in range(0, width):
        a = f.readline()
        h_t = round(FloatingPoint_to_float(a)/360*255)
        h_v[i][j] = h_t
        a = f.readline()  
        s_t = round(FloatingPoint_to_float(a)*255)
        s_v[i][j] = s_t
        a = f.readline()
        v_t = round(FloatingPoint_to_float(a)*255)
        v_v[i][j] = v_t 
f.close()
h_v = np.array(h_v)
s_v = np.array(s_v)
